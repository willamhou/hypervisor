...
pub fn run_multi_vm_devices_test() {
    hypervisor::log_info!("\n========================================\n");
    hypervisor::log_info!("  Multi-VM Device Isolation Test\n");
    hypervisor::log_info!("========================================\n\n");
    ...
    // addresses.
    hypervisor::log_info!("[MV-DEV] Test 1: Device registration isolation...\n");
    ...
    let uart_fr_1 = DEVICES[1]
        ...
        .unwrap_or(0);
    if uart_fr_0 == 0 {
        hypervisor::log_info!("[MV-DEV] FAILED: VM 0 UARTFR should be non-zero\n");
        return;
    }
    if uart_fr_1 != 0 {
        hypervisor::log_info!("[MV-DEV] FAILED: VM 1 should return 0 (no UART registered)\n");
        return;
    }
    hypervisor::log_info!("[MV-DEV] Test 1 PASSED\n\n");
    ...
    hypervisor::log_info!("[MV-DEV] Test 2: Independent GICD registration...\n");
    ...
    let vm0_gicd = DEVICES[0]
        ...
        .unwrap_or(0);
    if vm1_gicd == 0 {
        hypervisor::log_info!("[MV-DEV] FAILED: VM 1 GICD_CTLR should be non-zero\n");
        return;
    }
    if vm0_gicd != 0 {
        hypervisor::log_info!("[MV-DEV] FAILED: VM 0 should return 0 (no GICD registered)\n");
        return;
    }
    hypervisor::log_info!("[MV-DEV] Test 2 PASSED\n\n");

    // Test 3: Cross-check — VM 0 has UART, VM 1 has GICD, no cross-leak
    hypervisor::log_info!("[MV-DEV] Test 3: No cross-leak...\n");
    ...
    if vm1_uart_fr != 0 {
        hypervisor::log_info!("[MV-DEV] FAILED: VM 1 should return 0 for UART\n");
        return;
    }
    hypervisor::log_info!("[MV-DEV] Test 3 PASSED\n\n");
    ...
    hypervisor::log_info!("========================================\n");
    hypervisor::log_info!("  Multi-VM Device Isolation Test PASSED (3 assertions)\n");
    hypervisor::log_info!("========================================\n\n");
}
