...
pub fn run_decode_test() {
    hypervisor::log_info!("\n========================================\n");
    hypervisor::log_info!("  MMIO Instruction Decode Test\n");
    hypervisor::log_info!("========================================\n\n");

    // Test 1: ISS-based decode — 4-byte store, register x5
    hypervisor::log_info!("[DECODE] Test 1: ISS store word x5...\n");
    ...
    hypervisor::log_info!("[DECODE] Test 4: ISS store half x15...\n");
    ...
    hypervisor::log_info!("[DECODE] Test 5: Instruction STR W1, [X19]...\n");
    ...
    hypervisor::log_info!("[DECODE] Test 6: Instruction LDR W2, [X19]...\n");
    ...
    hypervisor::log_info!("[DECODE] Test 7: Instruction STRB W3, [X0]...\n");
    ...
    hypervisor::log_info!("[DECODE] Test 8: Instruction LDRH W4, [X1]...\n");
    ...
    hypervisor::log_info!("[DECODE] Test 9: Unsupported instruction...\n");
    let insn_add: u32 = 0x8b010000; // ADD X0, X0, X1
    if MmioAccess::decode(insn_add, 0).is_some() {
        hypervisor::log_info!("[DECODE] FAILED: should return None for ADD\n");
        return;
    }
    hypervisor::log_info!("[DECODE] Test 9 PASSED\n\n");

    hypervisor::log_info!("========================================\n");
    hypervisor::log_info!("  MMIO Instruction Decode Test PASSED (9 assertions)\n");
    hypervisor::log_info!("========================================\n\n");
}

fn assert_store(access: &MmioAccess, expected_reg: u8, expected_size: u8, label: &str) {
    if !access.is_store() {
        hypervisor::log_info!("[DECODE] FAILED: expected store for {}\n", label);
        return;
    }
    if access.reg() != expected_reg || access.size() != expected_size {
        hypervisor::log_info!("[DECODE] FAILED: wrong reg/size for {}\n", label);
        return;
    }
    hypervisor::log_info!("[DECODE] {} PASSED\n\n", label);
}

fn assert_load(access: &MmioAccess, expected_reg: u8, expected_size: u8, label: &str) {
    if !access.is_load() {
        hypervisor::log_info!("[DECODE] FAILED: expected load for {}\n", label);
        return;
    }
    if access.reg() != expected_reg || access.size() != expected_size {
        hypervisor::log_info!("[DECODE] FAILED: wrong reg/size for {}\n", label);
